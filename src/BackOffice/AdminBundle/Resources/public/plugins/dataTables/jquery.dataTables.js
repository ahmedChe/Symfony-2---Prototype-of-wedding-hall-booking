...
(/** @lends <global> */function( $ ) {
	...
	var _pluck = function ( a, prop, prop2 ) {
		...
		// Could have the Template in the loop for slightly smaller code, but speed
		// is essential here
		if ( prop2 !== undefined ) {
			for ( ; i<ien ; i++ ) {
				if ( a[i] && a[i][ prop ] ) {
	...
	function _fnSort ( oSettings )
	{
		...
		if ( _fnDataSource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
			 ...
			 *  }
			 * Basically we have a Template for each sorting column, if the data in that column is equal,
			 * Template the next column. If all columns match, then we use a numeric sort on the row
